...
export default class Artist extends React.Component<any, any> {
    ...
    componentDidMount()
    {
        ...
        const url = "http://ws.audioscrobbler.com/2.0/?method=artist.gettopalbums&artist=" + inputedArtist + "&api_key=a3057b25ed2acd6143c4543e74a2cbe6&format=json";
        axios.get(url)
            .then(res => {
        ...
        return (
            <section id="artistInfo" className="page-content__container">
                <div className="albums-page__title">
                    ...
                    <p>
                        <span>Добро пожаловать на страницу исполнителя <b>{ this.props.transferedArtistName }</b>!</span>
                ...
                <div className="albums-page__items_wrapper">
                    { this.state.artistAlbums.map((artistAlbums:any, i:any) => <a target="_blank" href={ artistAlbums.url } className="albums-page__single-item" key={ i }>
                        ...
                        <img src={ artistAlbums.image[2]['#text'] } alt={ "К сожалению, в нашей медиатеке нет изображений для данного альбома: " + artistAlbums.name + " отсутствует изображение" } />
                    </a>) }
                </div>
            </section>
        );
